...
**Implemented:** POST `/api/links` minimal API in `Program.cs`. Request body: `CreateLinkRequest` (LongUrl). Response: 201 Created with `CreateLinkResponse` (ShortCode, ShortUrl); Location header `/api/links/{shortCode}`. Validation: required LongUrl, max length 2048, absolute HTTP/HTTPS URL; invalid or missing returns 400 with `ErrorResponse` (Error). Short code generated via `IShortCodeGenerator` with up to 5 retries on collision (`ILinkRepository.ExistsAsync`). Base URL from config `ShortLink:BaseUrl` (default `http://localhost:5000`). Endpoint documented in Swagger via `AddEndpointsApiExplorer`/`AddSwaggerGen` (no `WithOpenApi` to avoid Microsoft.OpenApi version conflict with Swashbuckle). Optional Redis: after persist, mapping `<shortCode, longUrl>` written via `IDistributedCache.SetStringAsync`; failure caught and ignored (best-effort).

**Tests:** `tests/ShortLink.Api.UnitTests/CreateLinkEndpointTests.cs` — 7 tests: valid URL → 201 with ShortCode and ShortUrl; missing LongUrl → 400; empty LongUrl → 400; invalid scheme (ftp) → 400; LongUrl > 2048 → 400; invalid format → 400; valid URL → Location header present. Test host uses in-memory DB via `ConfigureTestServices` replacing `AppDbContext` with `UseInMemoryDatabase`.
